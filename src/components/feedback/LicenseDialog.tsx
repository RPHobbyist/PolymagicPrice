                    ...
                    {AGPL_LICENSE_TEXT.split(/\n\n+/).map((paragraph, index) => (
                        <p key={index} className="mb-4 text-justify leading-relaxed last:mb-0">
                            {paragraph.replace(/\n/g, " ").split(/(https?:\/\/[^\s<>]+)/g).map((part, i) => {
                                ...
                                // STRICT LOGIC: Double-verify protocol even after regex match
                                if (isUrl && (part.startsWith('http://') || part.startsWith('https://'))) {
                                    return <a key={i} href={part} target="_blank" rel="noopener noreferrer" className="text-foreground hover:underline font-medium">{part}</a>;
                                }
                                return part;
                            })}
                        </p>
